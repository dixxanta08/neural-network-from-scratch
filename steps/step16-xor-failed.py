    ...
    def train(self, inputs,epochs, targets, alpha):
        for epoch in range(0, epochs):
            ...
            deltas[-1] = (
                ...
                last_layer.activation(last_layer.z,True)
            )
            ...
            for layer_index in reversed(range(len(self.layers))):
                ...
                if layer_index == len(self.layers) - 1:
                    deltas[layer_index] = (
                        mse_derivate(targets, epoch_output) *
                        layer.activation(layer.z,True)
                    )
                else:
                    next_layer = self.layers[layer_index + 1]
                    weights_next = np.array([n.weights for n in next_layer.neurons])
...
neurons_layer_1 = [Neuron(np.array(rng.uniform(-1, 1, size=2)), 0) for _ in range(0,2)]
# neurons_layer_2 = [Neuron(np.array(rng.uniform(-1, 1, size=2)), 0) for _ in range(0,2)]
neurons_layer_3 = [Neuron(np.array(rng.uniform(-100, 100, size=2)), 0) for _ in range(0,1)]
layer_1 = Layer(neurons_layer_1, activation=relu)
# layer_2 = Layer(neurons_layer_2, activation=sigmoid)
layer_3 = Layer(neurons_layer_3, activation=sigmoid)

neural_network = NeuralNetwork([layer_1,layer_3])

neural_network.train(inputs=inputs, epochs=10000, targets=targets, alpha=0.1)
