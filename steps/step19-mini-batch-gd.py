...
class NeuralNetwork:
    ...
    def train(self, inputs, targets, batch_size=None):
        if batch_size is None:
            batch_size = len(inputs)
        for epoch in range(self.epochs):
            print(f"\n==============================\nEPOCH {epoch} of {self.epochs}\n==============================")
            rng = np.random.default_rng(int(42/(epoch+1)))
            indices = np.arange(len(inputs))
            rng.shuffle(indices)
            batch_indices = [indices[i:i + batch_size] for i in np.arange(0, len(indices), batch_size)]
            for batch_index in batch_indices:
                batch_input = inputs[batch_index]
                batch_target = targets[batch_index]
                result = self.forward(batch_input)
                self.outputs.append(result)
                loss_history = self.loss_function.forward(batch_target, result)
                self.loss_history.append(loss_history)

                self.backward(batch_target)
                self.compute_gradients()
                self.update_parameters()
                accuracy = self.calculate_accuracy(batch_target)
